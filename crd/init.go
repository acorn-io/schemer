...
type ApplyFunc func(...runtime.Object) error
...
func tagToColumn(f reflect.StructField) (apiextv1.CustomResourceColumnDefinition, bool) {
	...
	for _, col := range strings.Split(columnDef, ",") {
		k, v, _ := strings.Cut(col, "=")
		switch k {
		case "name":
			c.Name = v
		case "type":
...
func NamespacedType(name string) CRD {
	kindGroup, version, _ := strings.Cut(name, "/")
	kind, group, _ := strings.Cut(kindGroup, ".")
...
func NewFactoryFromClient(config *rest.Config, scheme *runtime.Scheme, apply ApplyFunc) (*Factory, error) {
